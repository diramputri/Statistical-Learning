...
lambda.best
#Compute test error
lasso.pred=predict(ridge,test.mat,alpha=1,s=lambda.best)
lasso.MSE=mean((test.set$Apps-lasso.pred)^2)
lasso.MSE
```

So far, our test MSEs are in the following order: Lasso << Ridge Regression < Least Squares
...
Lasso < Ridge Regression < Least Squares < PLS <<< PCR